...
public class InsertFiliere extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InsertFiliere frame = new InsertFiliere();
	...
	public InsertFiliere() {
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int ligne = TableF.getSelectedRow();
				if (ligne != -1) {
					String id = TableF.getModel().getValueAt(ligne, 0).toString();
					...
					try {
						...
						prepared.execute();
						ListeF();
						JOptionPane.showMessageDialog(null, "Filière a été supprimée avec succès !");
					...
					JOptionPane.showMessageDialog(null, "Aucune Filière sélectionnée !");
				}
			}
		});
		...
		JLabel lblNewLabel = new JLabel("Liste Filières :");
		...
		nomField.setBounds(175, 229, 205, 30);
		contentPane.add(nomField);
		nomField.setColumns(10);
		
		Dcombobox = new JComboBox();
		Dcombobox.setBounds(175, 270, 205, 30);
		contentPane.add(Dcombobox);
		Dcombobox.addItem("----- Sélectionner une option -----");
		rempComboboxD();
		...
		departementLabel.setBounds(66, 277, 99, 14);
		...
		ajouterbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String departement = Dcombobox.getSelectedItem().toString();
				
				String sql = "INSERT INTO filiere (nom, departement) VALUES (?, ?)";
				
				try {
					if (!nom.equals("") && !departement.equals("----- Sélectionner une option -----")){
						prepared = cnx.prepareStatement(sql);
						prepared.setString(1, nom);
						prepared.setString(2, departement);
						prepared.execute();
						ListeF();
						...
						JOptionPane.showMessageDialog(null, "Filière ajouté avec succès !");
		...
		nomLabel.setBounds(105, 236, 60, 14);
